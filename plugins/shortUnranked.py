...
async def startGameUnrankedShort(player1, player2):
    await db.Users.filter(id=player1.id).update(search=0, hp=25)
    await db.Users.filter(id=player2.id).update(search=0, hp=25)
    await player1.refresh_from_db()
    await player2.refresh_from_db()
    timeStat = int(time.time()) + 120
    match = await db.Matches.create(player1=player1.id, player2=player2.id, timeStat=timeStat, tournament='unranked_short')
    inMatch[player1.id] = []
    inMatch[player2.id] = []
...
async def calculate_win_unranked_short(match, player1, player2, winner):
