...
async def startGameUnrankedWar(player1, player2):
    ...
    match = await db.Matches.create(player1=player1.id, player2=player2.id, timeStat=timeStat, tournament='unranked_war', p1Mana=15, p2Mana=15)
    inMatch[player1.id] = []
    inMatch[player2.id] = []
    startText = "Первый ход определит монетка!"
    if random.randint(0, 100) >= 50:
        startText += "\n\nВыпала решка! Первый ход за {}".format(player2.username)
        await db.Matches.filter(id=match.id).update(status=2)
    else:
        startText += "\n\nВыпал орёл! Первый ход за {}".format(player1.username)
        await db.Matches.filter(id=match.id).update(status=1)
    try:
...
async def calculate_win_unranked_war(match, player1, player2, winner):
